...
SDLKey			SK::ToggleButton		= SDLK_SPACE;			// Button used in source to show the keyboard (not used if SK is triggered by mouse event)
...
void SK::HideIt()				{ if(SkipHideFlag) { SkipHideFlag=false; } else { CloseAnimation('A'); }; }
...
void SK::LoadOffsets() {
	// Image offset on screen (variable while the app is running)
		...
		TBox_off_X[0]=(SCREEN->w-img_text_box->w)/2;	TBox_off_Y[0]=(Keyb_off_Y[1]/2)-(img_text_box->h/2); // Overwritten when rendering
...
void SK::InitButtons() {
	...
	AddButton( 'H'	, "HideHead"	, 217	, 0		, 23	, 21	, SDLK_UNKNOWN		, ""	); // Head - Top Bar
	buttons_grp[1]=buttons_nr;
	
	buttons_grp[2]=buttons_nr+1;
	// removed char buttons
	buttons_grp[3]=buttons_nr;
	
	buttons_grp[4]=buttons_nr+1;
	AddButton( 'K'	, "1"			, 5		, 6		, 50	, 29	, SDLK_1		, "1" );
	AddButton( 'K'	, "2"			, 66	, 6		, 50	, 29	, SDLK_2		, "2" );
	AddButton( 'K'	, "3"			, 126	, 6		, 50	, 29	, SDLK_3		, "3" );
	AddButton( 'K'	, "4"			, 186	, 6		, 50	, 29	, SDLK_4		, "4" );
	
	AddButton( 'K'	, "5"			, 5		, 38	, 50	, 29	, SDLK_5		, "5" );
	AddButton( 'K'	, "6"			, 66	, 38	, 50	, 29	, SDLK_6		, "6" );
	AddButton( 'K'	, "7"			, 126	, 38	, 50	, 29	, SDLK_7		, "7" );
	AddButton( 'K'	, "8"			, 186	, 38	, 50	, 29	, SDLK_8		, "8" );
	
	AddButton( 'K'	, "9"			, 5		, 72	, 50	, 29	, SDLK_9		, "9" );
	AddButton( 'K'	, "0"			, 66	, 72	, 50	, 29	, SDLK_0		, "0" );
	AddButton( 'K'	, "Backspace"	, 126	, 72	, 48	, 28	, SDLK_BACKSPACE, ""  );
	AddButton( 'K'	, "Enter"		, 186	, 72	, 48	, 28	, SDLK_RETURN	, "\n");
...
/*		if(CallMode==1 && !Show_FirstFrame) { // FIXME FIXME FIXME - De facut evaluare butoane tastatura - FIXME FIXME FIXME
			switch(event->type) {
				case SDL_KEYDOWN:		{ if(ToggleButton && event->key.keysym.sym==ToggleButton) HideIt(); break; }
			}
		...
		// SEPARATED MODE rendering process
			...
			if(head_title.length()>0 && !HeadIsHidden) {
				...
				SFont_Write(SCREEN, img_font2, (SCREEN->w-SFont_TextWidth(img_font2, ht))/2, 
												Head_off_Y[0]+((img_head->h-SFont_TextHeight(img_font2))/2)+1,ht);
				RenderSurface(Head_off_X[0], Head_off_Y[0], img_head, SCREEN, NULL);
				TBox_off_Y[0]=(Keyb_off_Y[1]-img_head->h-img_text_box->h)/2+img_head->h;
				if(HeadIsHidding) {
					TBox_off_Y[0]+=Head_off_Y[0]/2;
					if(TBox_off_Y[0]<(Keyb_off_Y[1]-img_text_box->h)/2) TBox_off_Y[0]=(Keyb_off_Y[1]-img_text_box->h)/2;
				}
			} else { TBox_off_Y[0]=(Keyb_off_Y[1]-img_text_box->h)/2; }

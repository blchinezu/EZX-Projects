	...
	:UTIL_Dialog(UTIL_Dialog::DTLargeSize,true,p,"OpenFile Dialog",1,0)
{
   ...
   //setDlgContent(cw);
	...
	if(dir == "") browser->setDir(PATH_MMC);
...
QString FileDlg::getDirDirect()
	{ return getDirDirect(0, PATH_MMC, tr("Choose Directory")); }
...
QString FileDlg::getDirDirect(QWidget* p, const QString& loc, const QString& ttl)
{
	...
	FileDlg* dlg=new FileDlg(p,dd,"*",QDir::All |QDir::Hidden, QDir::Name | QDir::DirsFirst | QDir::IgnoreCase);
	dlg->setTitle(ttl);
	if(dlg->exec()==QDialog::Accepted)
		s=dlg->getDir();

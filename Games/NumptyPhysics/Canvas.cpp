...
Window::Window( int w, int h, const char* title, const char* winclass ){
	...
	printf("SDL_SetVideoMode: %ix%i\n",w, h);
	m_state = SDL_CreateRGBSurface( SDL_SWSURFACE, w, h, 16, 0, 0, 0, 0 );
	screen = SDL_SetVideoMode( 320, 240, 16, SDL_SWSURFACE);
	
//	printf("Cursor Load: %s\n",(f+"cursor.png").c_str());
//	cursor = IMG_Load((f+"cursor.png").c_str());
...
void Window::update( const Rect& r ){
	...
	extern int full_screen_x,full_screen_y,full_screen;
	

	
	Uint16 *pixels_fake = (Uint16 *)SURFACE(this)->pixels; 
	Uint16 *pixels_real = (Uint16 *)screen->pixels;	
...
#else
			for( int row=0; row<480; row+=2 ){
				for( float col=0; col<800; col+=2.5 ){
					//int e = (int)row;
					int r = (int)col;
					*pixels_real = pixels_fake[row*800+r];
					pixels_real++;
					}
			}
#endif
		} else {
			SDL_Rect srcrect = {full_screen_x, full_screen_y,full_screen_x+320, full_screen_y+240}, dest = {0,0};
			SDL_BlitSurface(SURFACE(this), &srcrect, screen, &dest);
			}
	SDL_Rect curso_pos;
	
	if (full_screen == 0){
		curso_pos.x = (int)(mouse_x/2.5f);
		curso_pos.y = mouse_y/2;
		} else {
		curso_pos.x = mouse_x - full_screen_x;
		curso_pos.y = mouse_y - full_screen_y;
		}
//	SDL_BlitSurface(cursor, NULL,screen, &curso_pos);

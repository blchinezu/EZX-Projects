...
bool CWords::splitDictLine(std::string text, tDictWord &dictword) 
{
...
bool CWords::wordInWord(const char * shortWord, const char * word6)
{
	...
	int il, is;		//longword and shortord counters
...
tDictWord CWords::getDictForWord(std::string &wrd)
{
	tWordMap::iterator it = m_mapAll.find(wrd);
	if (it == m_mapAll.end())
	{
		tDictWord wrdNotFound;
		return wrdNotFound;
	}
	return (*it).second;

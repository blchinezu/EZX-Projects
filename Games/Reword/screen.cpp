...
Screen::Screen (int w, int h) : _init(false)
{
	...
	w = 320;	//always
	h = 240;	//for gp2x
	_surface = SDL_SetVideoMode(w, h, 16, SDL_SWSURFACE);	//SDL_HWPALETTE|SDL_FULLSCREEN
#else
	_surface = SDL_SetVideoMode(w, h, 32, SDL_HWSURFACE|SDL_DOUBLEBUF);
#endif
...
//like SDL_Flip(_surface), but seems fractionally slower

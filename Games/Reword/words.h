...
struct tDictWord
{
	...
	tDictWord& operator=(const tDictWord &dw) {
...
//	bool operator < (const tDictWord &rhs)	//for sorting
//	{
//		return this->_word < rhs._word;
//	}

	bool operator () (const tDictWord &dw1, const tDictWord &dw2) const 
	{
		return dw1._word < dw2._word;
	}

	bool operator () (const tDictWord &dw1, const std::string &word) const 
	{
		return dw1._word < word;
	}
};

typedef std::map<std::string, tDictWord> tWordMap;	//map of random numbers and the (same length) words available 
...
public:
	...
	tDictWord getDictForWord(std::string &wrd);
...
protected:
	...
	bool splitDictLine(std::string line, tDictWord &dict);
	...
	tDictWord		m_word;					//current 6 letter word to find etc

...
void SK::ProcEvent(SDL_Event* event) {
	if(Show)
	{
		...
		if(CallMode==1 && !Show_FirstFrame) { // FIXME FIXME FIXME - De facut evaluare butoane tastatura - FIXME FIXME FIXME
			switch(event->type) {
				case SDL_QUIT:			{ HideIt(); break; }
				case SDL_KEYDOWN:		{ if(event->key.keysym.sym==SDLK_ESCAPE || event->key.keysym.sym==SDLK_RETURN) HideIt(); break; }
			}
...
bool SK::InitGraphics() { // FIXME - De terminat treaba cu comparatiile
	...
	img_text_box		= LoadImage(loc_text_box);
...
void SK::CloseAnimation(char x) {
	if(AllowAnimation) {
		if(!CloseAnim) {
			...
			if(!OpenAnim) {
				delta.start();
				if(x=='A'||x=='K') { Keyb_off_Y[0]=Keyb_off_Y[1]; SK::CapsLock=false; }
...
void SK::StopAnimation(char x) {
	if(AllowAnimation) {
		...
		if(x=='A'||x=='K') { Keyb_off_X[0]=Keyb_off_X[1];	Keyb_off_Y[0]=Keyb_off_Y[1]; SK::Mode='C'; }

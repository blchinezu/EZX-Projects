...
/*			if(event.type == SDL_JOYBUTTONDOWN || event.type == SDL_JOYBUTTONUP)
			...
			if(event.type == SDL_MOUSEBUTTONDOWN) EZX_ProcessTouch(event.button.x, event.button.y, 1); else
			if(event.type == SDL_MOUSEBUTTONUP)   EZX_ProcessTouch(event.button.x, event.button.y, 0);
		}
		...
		while(SDL_PollEvent(&event))
		{
			...
			{/*
				...
				case SDL_JOYBUTTONDOWN:
					WIZ_KeyboardEmulation( event.jbutton.button, event.jbutton.state );
					break;*/
				case SDL_MOUSEBUTTONDOWN: { EZX_ProcessTouch(event.button.x, event.button.y, 1); }	break;
				case SDL_MOUSEBUTTONUP:   { EZX_ProcessTouch(event.button.x, event.button.y, 0); }	break;
#ifndef _XBOX
				case SDL_QUIT:
				{
					return false;

...
bool PointOver(int cx, int cy, int x, int y, int w, int h) {	if((cx<x) || (cx>(x+w)) || (cy<y) || (cy>(y+h))) return false; return true; }
bool ClickOnTile(int mx, int my, int x, int y) { return PointOver(mx, my, POSX_TILES_LEFT+x*24, POSY_TILES_TOP+y*24, 24, 24); }
...
void MenuStart()
{
	...
	DrawText(0,0, buffer);
	DrawText((SCREEN_WIDTH - FONT_WIDTH * 20)/2, 205, "EZX Port by BruceLee");

#ifdef _WIN32
	DrawText(32, 224, "Press SPACE to start, ESC to exit");
#else
